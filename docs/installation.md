...
Options: `LEADLINE_VERSION=v0.8.1` pins a release, `LEADLINE_INSTALL_DIR` changes the destination (default `~/.local/bin`), `LEADLINE_SKIP_CHECKSUM=1` skips SHA256 verification (not recommended). `install.sh --help` prints the same.
...
Then add `%USERPROFILE%\.local\bin` to `Path` (System Properties → Environment Variables) and reopen the terminal. Pin a version by replacing `latest/download` with `download/v0.8.1`.
