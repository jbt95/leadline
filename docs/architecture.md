...
- `risk` ranks files by the `change-risk-v2` score: weight-renormalized components for complexity (20), CRAP (15), churn (20), impact (20), ownership concentration (10), and policy (15). Unknown components stay `null` and renormalize instead of zeroing; rows sort by score desc, path asc, and the build returns the full ranking (`--limit` caps terminal and agent-JSON rows only). The command is informational (exit `0`); gating is deferred to Milestone E.
