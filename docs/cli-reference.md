...
- `risk`: rank files by the explainable `change-risk-v1` score with exposed components (complexity 25, CRAP 20, churn 20, impact 20, ownership 15, policy always-`null` at weight 0). Unknown components stay `null` and the score renormalizes over the known weights; rows sort by score desc, path asc. Windows are HEAD-relative like `hotspots`; without Git, churn and ownership are `null` with `git_available: false`. Informational only (exit `0`); risk gating is deferred to Milestone E. See `docs/risk.md` for formulas and limitations. `--limit N` (default 10, `N >= 1`) caps rows; `--since 30d|90d|365d` selects the window (default `90d`); coverage flags merge LCOV/JaCoCo before the join.
...
- `duplication`: detect `tokens-v1` clones for the current state, or occurrence-level `new`/`existing`/`resolved` drift with `--base REV` (rename-mapped). Exceeding the token or comparison ceiling exits `3` with `complete: false`.
...
| `--format agent-json` | analyze, function, check, changed, diff, hotspots, risk, coupling, dependencies, impact, test-targets | Emit the compact agent-oriented JSON shape. Carries a `truncated` bool when budget flags drop entries. |
...
| `--limit N` | hotspots, risk | Keep at most `N` ranked files (`N >= 1`, default 10). Applies to terminal, JSON, and agent JSON; JSON carries `truncated`. |
