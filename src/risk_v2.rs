...
//!
//! V2 is a separate DTO and builder; `change-risk-v1` and its CLI bytes stay
//! byte-for-byte untouched. V2 activates when architecture rules exist.
...
pub const RISK_V2_SCHEMA_VERSION: u32 = 1;
pub const RISK_V2_MODEL: &str = "change-risk-v2";

pub const V2_WEIGHTS: [(&str, f64); 6] = [
    ("complexity", 20.0),
    ("crap", 15.0),
    ("churn", 20.0),
    ("impact", 20.0),
...
#[derive(Clone, Debug, PartialEq, Serialize)]
pub struct RiskV2Row {
    pub path: String,
    pub score: f64,
    pub components: RiskV2Components,
    pub raw: RiskV2Raw,
}

#[derive(Clone, Debug, PartialEq, Serialize)]
pub struct RiskV2Report {
...
pub fn build(
    ...
    let mut risks: Vec<RiskV2Row> = analysis
        .files
        .iter()
        .map(|file| {
            ...
            let components = RiskV2Components {
                complexity: Some(
                    100.0
    ...
    RiskV2Report {
        schema_version: RISK_V2_SCHEMA_VERSION,
...
fn weighted_score(components: &RiskV2Components) -> f64 {
    let mut weighted = 0.0_f64;
    let mut weights = 0.0_f64;
    for &(name, weight) in V2_WEIGHTS.iter() {
