...
/// means [`DEFAULT_PORT`], `0` asks the OS for a free port) and
...
pub fn serve_http(host: &str, port: u16) -> crate::Result<()> {
    let (listener, actual, fell_back) = bind(host, port)?;
    if fell_back {
    ...
    crate::http::serve(listener, |request| {
        let (status, body) = route_http(request);
        (status, JSON_CONTENT_TYPE, body.into_bytes())
    });
    Ok(())
}

/// Route one HTTP request to the shared JSON-RPC handler, after the transport
/// guards accepted its `Host` and `Origin`.
fn route_http(request: &Request) -> (u16, String) {
...
fn tool_explain_metric(params: &serde_json::Value) -> Result<serde_json::Value, (i64, String)> {
    ...
    let definition = match name.as_str() {
        "cyclomatic" => {
            "Cyclomatic complexity (default): every function starts at 1. Java: add 1 for each `if`, loop, non-default switch case, ternary expression, `&&`, `||`, and each `->` (lambda/switch arrow, counted in the enclosing function); `catch` adds nothing. JavaScript/TypeScript: add 1 for each `if`, loop, `catch`, non-default switch case, ternary expression, `throw`, `&&`, and `||` (`??` adds nothing). `else`, `finally`, and default cases add nothing. Nested functions are scored independently. Zig body-bearing function declarations and block-bearing test declarations are scored independently; add 1 for each `if`, loop, `switch_case` (including an `else` arm), `try`, `catch`, `orelse`, `and`, and `or`. A `switch` itself, explicit labeled statements, labeled `break`/`continue` jumps, and a non-case `else` add nothing."
        }
        "cognitive" => {
            "Cognitive complexity (default): add `1 + current nesting` for `if`, loops, `catch`, `switch`, and ternary expressions. An `else if` adds 1 and continues the chain; a final `else` adds 1; a labeled `break`/`continue` adds 1. For logical expressions the first `&&`/`||` adds 1 and each operator change adds 1. Lambdas, arrows, and nested functions are scored independently and do not raise the enclosing score. Direct self-recursion adds 1; mutual cycles are not scored. Zig body-bearing function declarations and block-bearing test declarations use the same policy: `if` and loops add `1 + current nesting`; a `switch` adds `1 + current nesting` while each `switch_case` (including an `else` arm) adds 0; `try` adds 0; `catch` and `orelse` add `1 + current nesting` and increase structural nesting; each explicit labeled statement or labeled `break`/`continue` jump adds 1; and the first `and`/`or` in a logical sequence adds 1, with each operator change adding 1."
        }
        "halstead" => {
            "Halstead metrics (default): leaf operator tokens and control keywords are operators; identifiers, literals, `this`, `super`, `true`, `false`, and `null` are operands. Distinct values are source byte slices within one function. Vocabulary is n1 + n2, length is N1 + N2, volume is length * log2(vocabulary), difficulty is (n1 / 2) * (N2 / n2), effort is difficulty * volume. Zero denominators produce zero, never NaN. For Zig, named `boolean`, `builtin_type`, `builtin_identifier`, `string`, `character`, and `multiline_string` wrappers are counted once as operands; `comptime_int`, `comptime_float`, `anyframe`, `noreturn`, `undefined`, and `unreachable` are operands, while Zig-only keyword tokens are operators."
        }
        "maintainability" => {
