...
- `GET /api/telemetry` serves the opt-in local metrics store to the stats page (`ok` with counters, summaries, and gauges, `disabled` when `LEADLINE_METRICS_DIR` is unset, or `error` when the store exceeds 1 MiB), and the page renders a Telemetry section from it: invocation counts, gate failures, mean latency per operation, live CPU and memory gauges, and gate finding counts. Histogram rows also carry their bucket bounds so the page can show p50/p90/p99 latency, p90 CPU, and p90 peak memory per operation with Prometheus-style interpolation, plus calls-by-outcome stacked bars. Every five seconds the server samples the sampler's live CPU/RSS gauges plus store-wide totals into a capped 720-point ring (one hour) served as the `series` array, backing CPU, memory, and invocation-rate timeseries of what real invocations consumed. The endpoint refuses stores above 1 MiB, adds bucket bounds to histogram rows, and appends the in-memory `series` ring; it never mutates the persisted store.
- The stats dashboard navigates through a shadcn-style sidebar where every item is a hash route (`#/overview` … `#/policy`): one section renders at a time, each URL is deep-linkable, and the browser back/forward buttons work. The sidebar collapses to an icon rail on desktop (persisted in localStorage), becomes an overlay drawer on mobile, shows the error-count badge on Policy, and the command palette jumps to routes. The scrollspy observer is gone — the active item derives from the route — and the palette resets its query during render instead of in an effect. The co-change-pairs table also shows directional, reverse, and Jaccard values.
- The dashboard bundle carries a source fingerprint (`web/dist/.src-hash`, written by `npm run build`) that the offline suite checks, so a `web/src` change without a rebuild fails loudly instead of serving a stale page.
- The sidebar fills the whole viewport height and absorbs the former top bar: brand plus project name and commit on top, a Session group for search, refresh, and JSON download, and the refresh status in the footer (a sticky drawer opener replaces the bar on mobile). Thin sections gained substance from analyzer values they already had: Measures adds dependency-edge, clone-group, policy-violation, and risk-model badges plus a largest-files table; Coupling adds a ranked co-change-pairs table; Trends renders a snapshot-history table and, with no history, a current-position baseline card; Policy groups violations by rule with worst severity and count.
- Polish: measure tiles no longer stutter identical label/caption text, the risk-model value renders at text size instead of a numeral badge, and the top-risk bars use a track template that cannot starve — the fixed-maximum name column was absorbing all free space and collapsing every bar to a dot. The quality-gate verdict animation now replays on every refresh (keyed by readout time), so the instrument visibly settles when new numbers land.

### Changed

- The stats dashboard no longer truncates its two full-length tables: the files table on Measures and the risk-component list on Hotspots render every row in 200-row pages with paging controls, instead of stopping at the top 10 and top 8.
...
- C language support: `analyze`, `check`, `changed`, duplication tokenization, and dependency analysis handle `.c` sources. Quoted includes resolve relative to their source file, while angle-bracket and macro-built includes stay ignored.
...
- `unused` treats every C and C++ translation unit (`.c`, `.cpp`, `.cc`, `.cxx`) as an entry point, because a build compiles those files rather than including them, so a compiled source is never reachable over include edges. Headers stay subject to reachability, where an unreferenced header is still a candidate row.
